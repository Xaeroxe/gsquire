...
fn main() {
	...
	let discord = Discord::from_bot_token(
		include_str!("bot_key.txt").trim()
	).expect("Login failed.");

	discord.connect().expect("Websocket login failed.");
	for server in discord.get_servers().expect("Getting servers failed") {
		clear_old_channels(&discord, server);
	}
	println!("Job finished.");
}

fn clear_old_channels(discord : &Discord, server: ServerInfo) {
	...
	if days_old == 6 {
		...
		match get_warning(discord, channel) {
			Some(warning) => {
				...
				//23 is intentional here as exactly 24 hours almost never happens.
				if (Local::now() - msg_time).num_hours() >= 23 {
...
fn get_warning(discord : &Discord, channel : &PublicChannel) -> Option<Message> {
	...
	else
	{
		...
		if last_msg_vec.len() == 0 {
			...
			if last_msg.author.id == ME && last_msg.content == include_str!("delete_warning.txt") {
